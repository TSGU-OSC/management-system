  ...
  <div>
    ...
    <el-row>
      <el-col :span="24">
        <el-row :gutter="25">
          ...
          <!-- <el-col :span="5" style="margin-bottom: 20px">
           <el-cascader  placeholder="请选择地区" v-model="queryInfo.province" :options="optionsnative_place"  />
          </el-col> -->
        ...
        <el-row :gutter="25" style="margin-bottom: 20px">
          ...
          <el-col :span="4" v-if="this.$store.state.user.role !== 0">
        ...
        <el-row :gutter="25">
          ...
          <el-col :span="2.5" style="margin-bottom: 10px">
            <el-button type="primary" v-if="this.$store.state.user.role !== 0" icon="el-icon-plus"
              @click="addDialogVisible = true">添加用户</el-button>

            <el-dropdown v-if="this.$store.state.user.role !== 0">
              <el-button type="success" icon="el-icon-document">Excel</el-button>
              <template>
                <el-dropdown-menu>
                  <el-dropdown-item>
                    <el-upload action="/api/excel/input" accept=".xlsx,.xls" :show-file-list="false"
      ...
      <el-col :span="24">
        ...
        <el-table :data="userList" border stripe>
          ...
          <el-table-column prop="updateTime" label="更新时间" width="160"
            v-if="this.$store.state.user.role !== 0"></el-table-column>
          <el-table-column prop="updateUser" label="更新者ID" width="100"
            v-if="this.$store.state.user.role !== 0"></el-table-column>
          <el-table-column label="操作" :width=this.width fixed="right">
            ...
            <template v-slot:default="scope">
              ...
              <el-button type="primary" size="mini" icon="el-icon-edit" @click="showEditDialog(scope.row)"
                v-if="$store.state.user.role !== 0"></el-button>
              <!-- 删除按钮-->
              ...
              <el-button type="success" size="mini" icon="el-icon-view"
                @click="seeUserIntroduction(scope.row)"></el-button>
            </template>
          </el-table-column>
        </el-table>
      </el-col>
    ...
    <el-dialog title="修改用户" :visible.sync="editDialogVisible" width="65%">
      ...
      <el-form :model="editForm" label-width="80px" :inline="true">
        <el-form-item label="学号" prop="code" v-if="this.$store.state.user.role == 2">
        ...
        <el-form-item label="地区" prop="province">
          <el-cascader placeholder="请选择地区" v-model="editForm.province" :options="optionsnative_place" />
        </el-form-item>
        <el-form-item label="职位" prop="duty">
    ...
    <!--查看用户的对话框-->
    <el-dialog title="查看用户个人介绍" :visible.sync="seeDialogVisible" width="80%">
      <!--内容主体区域-->

      <quill-editor v-if="this.$store.state.user.role >= 1" ref="myQuillEditor" v-model="editForm.introduction"
      ...
      <div v-else v-html="editForm.introduction"></div>
      <!--底部按钮区域-->
      <span slot="footer" class="dialog-footer">
        <el-button @click="seeDialogVisible = false">返回</el-button>
        <el-button type="primary" @click="editUser" v-if="this.$store.state.user.role >= 1">修改</el-button>
      </span>
    </el-dialog>
  </div>
</template>
